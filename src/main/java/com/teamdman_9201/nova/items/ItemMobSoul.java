...
public class ItemMobSoul extends Item {
    ...
    @Override
    public boolean onItemUse(ItemStack soul, EntityPlayer player, World world, int x, int y, int z, int side, float xOffset, float yOffset, float zOffset) {
        ...
        if(side == ForgeDirection.UP.ordinal() && (block instanceof BlockFence || block instanceof BlockWall)) {
            spawnY += 0.5;
        }
