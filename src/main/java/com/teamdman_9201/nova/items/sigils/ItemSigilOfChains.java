...
public class ItemSigilOfChains extends EnergyItems {
    ...
    @Override
    public boolean itemInteractionForEntity(ItemStack sigil, EntityPlayer player, EntityLivingBase mob) {
        if(player.worldObj.isRemote)
            return true;
        if(mob instanceof IHoardDemon || mob instanceof EntityWither || mob instanceof EntityDragon || mob instanceof EntityPlayer || mob instanceof IBossDisplayData || mob.isDead || mob.getHealth() < 0.5f)
            return false;
        ...
        for(int i = 0; i < 10; i++) {
            SpellHelper.sendIndexedParticleToAllAround(world, mob.posX, mob.posY, mob.posZ, 20, world.provider.dimensionId, 1, mob.posX, mob.posY, mob.posZ);
        }
        ...
        soul.setStackDisplayName((tag.hasKey("name") ? tag.getString("name") + "'s" : tag.getString("id")) + " Soul");
